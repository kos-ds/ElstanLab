...
namespace ElstanLab.Pages.ReportPage.Reports
{
    public class AVReportBuilder : BaseReportBuilder
    {
        public override string Build()
        {
            ...
            AVSnapshot s = LabStorage.CurrentAV;
...
");
        ...
        private void BuildCharts1(StringBuilder sb, AVSnapshot s)
        {
            if (s.Times == null || s.Times.Count < 2)
            {
                sb.AppendLine(@"
<div class='divcaption'>5. Графики записи</div>
<p style='color:#888;'>Недостаточно данных для построения графиков.</p>
");
                return;
            }

            // Относительное время (чтобы ось X начиналась с 0)
            double t0 = s.Times[0];

            // Формируем JS-массивы
            ...
            sb.AppendLine(@"
<div class='divcaption'>4. Графики записи (60 сек)</div>

<div style='margin-bottom: 25px;'>
    <canvas id='chartU' height='220'></canvas>
</div>

<div style='margin-bottom: 30px;'>
    <canvas id='chartI' height='220'></canvas>
</div>
...
(function() {
    ...
    new Chart(document.getElementById('chartU'), {
        ...
        options: {
            ...
            plugins: {
                ...
                title: { display: true, text: 'Напряжение по фазам', font: { size: 15 } }
            }
        }
    });
    ...
    new Chart(document.getElementById('chartI'), {
        ...
        options: {
            ...
            plugins: {
                ...commonOptions.plugins,
                title: { display: true, text: 'Ток по фазам', font: { size: 15 } }
            }
        }
    });
})();
...
");
        ...
        private void BuildCharts0(StringBuilder sb, AVSnapshot s)
        {
            if (s.Times == null || s.Times.Count < 2)
            {
                sb.AppendLine(@"
...
");
            ...
            sb.AppendLine(@"
<div class='divcaption'>5. Графики записи (60 сек)</div>
...
");
        ...
        private void BuildCharts(StringBuilder sb, AVSnapshot s)
        {
            if (s.Times == null || s.Times.Count < 2)
            {
                sb.AppendLine(@"
...
");
        ...
        private void BuildMeasuredData(StringBuilder sb, AVSnapshot s)
        {
            sb.AppendLine(@"
<table class='compact'>
...
");
        ...
        private void BuildStabilityTable(StringBuilder sb, AVSnapshot s)
        {
            const double limit = 20.0;   // % — как в анализе
...
");
        ...
        private void BuildConclusion(StringBuilder sb, PassportModel p, AVSnapshot s)
        {
...
");
            ...
            sb.AppendLine($@"
<div class='{cls}'>
По результатам испытания индуцированным напряжением в течение 60 секунд:<br>
среднее напряжение составило U<sub>ср</sub> = {s.Uavg:F1} В,<br>
